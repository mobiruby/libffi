...
if [ -z "${OUTPUT_DIR}" -o -z "${MIN_IOS_VERSION}" -o -z "${SDK_IOS_VERSION}" ]; then
  echo "error"
  exit 1
fi
[ -z "${PLATFORM_IOS}" ]; PLATFORM_IOS=`xcode-select -print-path`/Platforms/iPhoneOS.platform/
[ -z "${PLATFORM_IOS_SIM}" ]; PLATFORM_IOS_SIM=`xcode-select -print-path`/Platforms/iPhoneSimulator.platform/
